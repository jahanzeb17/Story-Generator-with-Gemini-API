...
# Function to stream Gemini Pro response
def stream_gemini_response(model, prompt, temperature):
    model.temperature = temperature
    for chunk in model.stream(prompt):  # Use plain string as prompt
        if hasattr(chunk, "content") and chunk.content:
            yield chunk.content

# Streamlit app UI
st.header("✨ AI-Powered Story Generator", divider="rainbow")
st.subheader("with Gemini API and LangChain (Streaming Enabled)")
st.write("(Developer: Jahanzeb)")
text_model_pro = load_model()
...
if st.button("Generate my story"):
    try:
        ...
        st.subheader("Your Story")
        with st.spinner("Streaming your story..."):
            streamed_story = st.write_stream(stream_gemini_response(text_model_pro, prompt, temperature))
